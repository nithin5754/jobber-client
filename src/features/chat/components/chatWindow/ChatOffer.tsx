...
const messageOffer:IOffer=message.offer as IOffer
  ...
  return (
    ...
    <div className="w-full">
        <div
            className="border-grey flex flex-row justify-between border-b bg-[#fafafa] p-4 text-sm font-bold md:text-base">
            <span className="">{messageOffer?.gigTitle}</span>
            <span>${messageOffer?.price}</span>
        </div>
        ...
        <div className="border-grey flex flex-row gap-x-2 border-b px-4 py-3 text-sm font-bold md:text-base">
            <FaRegClock className="self-center" />{messageOffer?.deliveryInDays}Day{parseInt(`${messageOffer.deliveryInDays}`)>1?'s':''}Delivery
        </div>
        <div className="relative top-[5%] mr-3 flex flex-row justify-end gap-4">
             ...
             disabled={messageOffer.accepted||messageOffer.cancelled} label="Cancel Offer"   onClick={()=>handleUpdateBuyerOffer(`${message.id}`,'cancelled',messageOffer)}/>
...

          {
            seller&&seller.id!==message.sellerId&&(
              ...
              }`}                disabled={messageOffer.accepted||messageOffer.cancelled} label="Accept Offer"  onClick={()=>handleUpdateBuyerOffer(`${message.id}`,'accepted',messageOffer)}/>
            )
          }
