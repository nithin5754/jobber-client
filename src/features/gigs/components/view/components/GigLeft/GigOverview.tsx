...
const GigOverview:FC = ():ReactElement =>{
  const {gig,isLoading,isSuccess}=useContext(GigContext)
  return (
    <div className="relative  flex max-h-[600px] h-[600px] cursor-pointer justify-center bg-[#F5F5F5]">
 {
  !isLoading&&isSuccess&&(
  <LazyLoadImage src={gig.coverImage} alt="Gig Image"
        className="w-full h-full object-cover transition-all duration-500 hover:scale-105"
        placeholderSrc="https://placehold.co/330x220?text=Profile+Image" effect="opacity" />
