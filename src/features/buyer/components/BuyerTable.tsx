...
const Table: FC<IOrderTableProps> = ({ type, orders, orderTypes }): ReactElement => {
  return (
          ...
          <>
            <thead className="border-grey border-b text-xs uppercase text-gray-700 sm:[&>*:not(:first-child)]:hidden">
              {orders.map(() => (
                <tr
                  key={uuidv4()}
                  className="mb-1 flex flex-col flex-nowrap bg-custtext-customPurple  text-white sm:mb-0 
            ...
            orders.map((order:IOrder)=>(
              ...
              >
                ...
                <td className="p-3 text-left">
                  <div className="grid">
                    <Link to={`/orders/${order.orderId}/activities`} className="truncate text-sm font-normal hover:text-customPurple ">
                 {order.gigBasicTitle}
                    </Link>
                  </div>
                </td>
