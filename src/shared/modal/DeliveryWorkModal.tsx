import { LazyExoticComponent, FC, lazy, ChangeEvent, ForwardRefExoticComponent, useRef, useState } from 'react';
import { FaCircleNotch, FaRegFile, FaTimes, FaPaperclip } from 'react-icons/fa';

import { IButtonProps, ITextInputProps } from '../shared.interface';
import { bytesToSize, showErrorToast, showSuccessToast } from '../utils/utils.service';
import { IModalBgProps, IModalProps } from './interfaces/modal.interface';
...
const ModalBg: LazyExoticComponent<FC<IModalBgProps>> = lazy(() => import('src/shared/modal/ModalBg'));

const TextAreaInput: FC<ITextInputProps> = lazy(() => import('../inputs/TextAreaInput'));
...
const DeliveryWorkModal: FC<IModalProps> = ({ order, onClose }) => {
  ...
  const deliverWork = async (): Promise<void> => {
    try {
      ...
      if(selectedFile){
        ...
        formData.append('message', completedWork.message);
        formData.append('deliveryFile',selectedFile);
        formData.append('fileType', completedWork.fileType);
        formData.append('fileSize', completedWork.fileSize.toString()); 
        formData.append('fileName', completedWork.fileName);
  
        await deliverOrder({ orderId: `${order?.orderId}`, formData: formData });
